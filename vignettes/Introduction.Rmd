...
Where,
  ...
  $\lambda_{n}$ is the is the singular value for IPC $n$ and correspondingly $\lambda_{n}^{2}$  is its eigen value;
  $\gamma_{in}$ is the eigenvector value for $i$th genotype;
  $\delta_{jn}$  is the eigenvector value for $j$th environment;
...
The most stable genotype need not necessarily be the high yielding genotype. Hence, simultaneous selection indices (SSIs) have been proposed for the selection of the most stable as well as highest yielding genotypes.
...
In `AMMIStbP`, the above expression has been implemented for all the stability parameters ($SP$) including ASTAB.
...
The function `SSI` implements both these indices in `AMMIStbP`. Further, for each of the stability parameter functions, the simultaneous selection index is also computed by either of these functions as specifiend by the argument `ssi.method`. 
...
suppressMessages(cit <- citation("AMMIStbP")) # eval = FALSE
